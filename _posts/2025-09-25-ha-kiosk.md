...
- Supports a handy **three-finger tap to reload** when Wi-Fi or HA hangs
  (via the Chromium alternative; with TouchKio you can trigger a reload through its MQTT controls instead)
...
> On Ubuntu these files live on the boot partition: edit `/boot/firmware/config.txt`
> and `/boot/firmware/cmdline.txt`. You can edit them on the flashed SD card from
> another machine, or on the Pi itself after first boot - they only take effect on reboot.
...
gpu_mem=128
# --- Overclock (optional) ---
# gpu_freq=600 and arm_freq=2000 exceed stock speeds (500/1500 MHz). Some Pi 4
# boards fail to boot or run unstable with these - remove both lines if that happens.
# arm_boost=1 only affects newer Pi 4B revisions and is redundant while arm_freq is set explicitly.
gpu_freq=600
arm_freq=2000
arm_boost=1
arm_64bit=1
...
```plain
# Ubuntu already ships a working cmdline.txt on the boot partition -
# only add extra parameters here if needed, e.g. to quiet the boot console:
quiet loglevel=3
```
...
# 98304 131072 196608 = 384/512/768 MB, suitable for an 8 GB Pi.
...
```sh
sudo apt install -y labwc swayidle wlopm wlr-randr wayland-utils libva-drm2 libinput-tools squeekboard curl jq
```
...
sudo -u kiosk tee /home/kiosk/.config/systemd/user/labwc.service <<'EOF'
[Unit]
Description=Labwc - Wayland compositor
Documentation=man:labwc(1) man:labwc-config(5)
...
sudo -u kiosk XDG_RUNTIME_DIR=/run/user/$(id -u kiosk) systemctl --user --now enable labwc.service
```

### Configure Labwc Session

Labwc is configured via an `autostart` script (output setup & idle handling):
...
>   ```
> - `1280x800`: your screen resolution - check possible mode with:
>   ```sh
>   sudo -u kiosk XDG_RUNTIME_DIR=/run/user/$(id -u kiosk) \
>   WAYLAND_DISPLAY=wayland-0 wlr-randr
>   ```
...
### Output setup: resolution & rotation ###
wlr-randr --output HDMI-A-1 --mode 1280x800 --transform 270

### Idle handling: display off after 60s idle, back on on touch ###
swayidle -w \
  timeout 60 'wlr-randr --output HDMI-A-1 --off' \
...
sudo -u kiosk XDG_RUNTIME_DIR=/run/user/$(id -u kiosk) systemctl --user restart labwc.service
```

> Notes:
>
> - labwc creates the Wayland socket as `wayland-0` - the services below use
>   `WAYLAND_DISPLAY=wayland-0` accordingly.
> - If screen or touch rotation is not aligned, a reboot is needed.

### Rotate Touch Input

This panel's touch axes need a libinput calibration matrix, so
libinput handles it independent of labwc:

```sh
sudo tee /etc/udev/rules.d/99-libinput-touch.rules <<'EOF'
...
> - `0 1 0 -1 0 1` (270 deg) is verified for this panel. If a swipe still
>   comes out rotated, try `0 -1 1 1 0 0` (90 deg) or `-1 0 1 0 -1 1` (180).
> - A reboot is needed for the property to reach the touch device.
...
> - `wayland-0`: find correct DISPLAY index - check possible option with:
>   ```sh
...
After=graphical.target labwc.service
...
ExecStart=/snap/bin/chromium --kiosk \
  ...
  --disable-extensions-except=/home/kiosk/reload-on-touch \
  --enable-zero-copy \
  --disable-gpu-sandbox \
  --no-sandbox \
...
- **Slow Snap apps**: prefer native packages where possible -
  note the apt `chromium-browser` package is a snap under the hood,
  so use the Mozilla-repo Firefox, TouchKio or Butler instead.
- **Check service logs on problems**:
  - `sudo -u kiosk XDG_RUNTIME_DIR=/run/user/$(id -u kiosk) journalctl --user -u labwc.service -f`
